...
Exemple:
  url-shortener stats --code="xyz123"`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if shortCodeFlag == "" {
			fmt.Print("➡️  Renseigner le code de l'URL : ")
			reader := bufio.NewReader(os.Stdin)
			shortCodeFlag, err = utils.ReaderLine(reader)
			if err != nil {
		...
		if err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				log.Fatalf("‼️  Aucun lien trouvé avec le code court : %s", shortCodeFlag)
			}
			log.Fatalf("‼️  Erreur lors de la récupération des statistiques de l'URL : %v", err)
			os.Exit(1)
		}
...
func init() {
	...
	StatsCmd.Flags().StringVarP(&shortCodeFlag, "code", "c", "", "Code de l'URL courte afin d'obtenir ses statistiques")

	// TODO Marquer le flag comme requis
	StatsCmd.MarkFlagRequired("url")
